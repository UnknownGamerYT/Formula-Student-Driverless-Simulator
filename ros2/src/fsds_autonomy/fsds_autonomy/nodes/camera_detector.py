...
class CameraDetector(Node):
    def __init__(self) -> None:
        ...
        self.declare_parameter("stereo_min_confidence", 0.30)
        ...
        if self.detector.available:
            self.get_logger().info("YOLO detector loaded for camera cones")
            ...
            self.get_logger().warn("YOLO detector unavailable; using HSV cone color fallback")
    ...
    def draw_overlay(self, frame, detections: list[CameraDetection], camera_name: str, backend: str):
        ...
        cv2.putText(
            debug,
            "blue/yellow=cone boundaries  cyan=current line  magenta=optimal line",
            (10, debug.shape[0] - 12),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.45,
            (255, 255, 255),
    ...
    def draw_status_panel(self, frame, camera_name: str, backend: str, detection_count: int) -> None:
        state = self.latest_race_state
        lines = [
            f"{camera_name} | {backend} | detections={detection_count} stereo={len(self.latest_stereo_cones)}",
            (
    ...
    def draw_path_overlay(self, frame, detections: list[CameraDetection], camera_name: str) -> None:
        ...
        boundary_groups = self.map_boundary_points(pose) if pose is not None else {}
        if sum(len(points) for points in boundary_groups.values()) < 4:
        ...
        current_path = self.latest_track.racing_line or self.latest_track.centerline
    ...
    def draw_projected_path(
        ...
        for path_point in global_points:
            ...
            if local_x <= 0.5 or local_x > self.overlay_max_range or abs(local_y + local_y_offset) > 12.0:
